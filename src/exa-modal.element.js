class ModalElement extends HTMLBaseElement {
  constructor() {
    super();
    ...
    this.container = this.querySelector('.exa-modal-dialog');
  }
  ...
  childrenAvailableCallback() {
    if (!this.id) {
      throw new Error('`modal element` requires an id');
    }
    ...
    if (!this.container) {
      ...
      cont.classList.add('exa-modal-dialog');
  ...
  documentClose(event) {
    if (!this.findAncestorByClass(event.target, 'exa-modal-dialog') && event.target !== this.triggerBtn) {
      this.close();
    }
  }
...
customElements.define('exa-modal', ModalElement);
