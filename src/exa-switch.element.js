class SwitchElement extends HTMLBaseElement {
  ...
  childrenAvailableCallback() {
    ...
    if (this.inputs.length !== 2 || this.inputs[0].type !== 'radio') {
      throw new Error('`Switch Element` requires two inputs type="checkbox"');
    }
  ...
  disconnectedCallback() {
    this.removeEventListener('switch.toggle', this.toggle, true);
  ...
  switch() {
    ...
    if (this.newActive === 1) {
      ...
      // Dispatch the "switch.on" event
      this.dispatchCustomEvent('switch.on');
    } else {
      ...
      // Dispatch the "switch.off" event
      this.dispatchCustomEvent('switch.off');
    }
...
customElements.define('exa-switch', SwitchElement);
