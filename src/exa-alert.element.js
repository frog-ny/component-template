class Alert extends HTMLElement {
  ...
  connectedCallback() {
    // Trigger show event
    this.dispatchCustomEvent('exa.alert.show');
    this.setAttribute('role', 'alert');
    this.classList.add('exa-alert--show');
    ...
    if (this.hasAttribute('dismiss') || this.hasAttribute('acknowledge') || (this.hasAttribute('href') && this.getAttribute('href') !== '')) {
      if (!this.querySelector('button.exa-alert--close') && !this.querySelector('button.exa-alert-button--close')) {
  ...
  attributeChangedCallback(attr, oldValue, newValue) {
    switch (attr) {
      ...
      case 'href':
        ...
        } else if (this.firstElementChild.tagName && this.firstElementChild.tagName.toLowerCase() !== 'button' && this.firstElementChild.classList.contains('exa-alert-button--close')) {
          this.appendCloseButton.bind(this)();
        }
        break;
      case 'auto-dismiss':
  ...
  buttonCloseFn() {
    this.dispatchCustomEvent('exa.alert.buttonClicked');
    if (this.href) {
  ...
  close() {
    this.dispatchCustomEvent('exa.alert.close');
    this.addEventListener('transitionend', () => {
      this.dispatchCustomEvent('exa.alert.closed');
      this.parentNode.removeChild(this);
    });
    this.classList.remove('exa-alert--show');
  }
  ...
  appendCloseButton() {
    ...
    if (this.hasAttribute('dismiss')) {
      closeButton.classList.add('exa-alert--close');
      closeButton.innerHTML = '<span aria-hidden="true">&times;</span>';
      closeButton.setAttribute('aria-label', this.textClose);
    } else {
      closeButton.classList.add('exa-alert-button--close');
    ...
    if (this.autoDismiss > 0) {
      ...
      setTimeout(() => {
        self.dispatchCustomEvent('exa.alert.buttonClicked');
        if (self.href) {
